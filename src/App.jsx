...
const TARGET_SPOTS = [
  { id: "left-desk", x: 20, y: 52 },
  { id: "front-desk", x: 52, y: 49 },
  { id: "right-monitor", x: 88, y: 48 },
  { id: "low-left", x: 34, y: 66 },
  { id: "center", x: 50, y: 38 },
  { id: "right-desk", x: 76, y: 56 },
];

const TOMATO_SPAWNS = [
  { x: 10, y: 14 }, { x: 30, y: 24 }, { x: 70, y: 16 },
  { x: 86, y: 30 }, { x: 12, y: 74 }, { x: 55, y: 52 },
];
...
function CrosshairIcon({ type }) {
  if (type === 1) return <svg viewBox="0 0 100 100"><circle cx="50" cy="50" r="30" fill="none" stroke="currentColor" strokeWidth="6"/><line x1="50" y1="5" x2="50" y2="28" stroke="currentColor" strokeWidth="6"/><line x1="50" y1="72" x2="50" y2="95" stroke="currentColor" strokeWidth="6"/><line x1="5" y1="50" x2="28" y2="50" stroke="currentColor" strokeWidth="6"/><line x1="72" y1="50" x2="95" y2="50" stroke="currentColor" strokeWidth="6"/></svg>;
  if (type === 2) return <svg viewBox="0 0 100 100"><rect x="22" y="22" width="56" height="56" fill="none" stroke="currentColor" strokeWidth="6" transform="rotate(45 50 50)"/><line x1="50" y1="0" x2="50" y2="100" stroke="currentColor" strokeWidth="4"/><line x1="0" y1="50" x2="100" y2="50" stroke="currentColor" strokeWidth="4"/></svg>;
  if (type === 3) return <svg viewBox="0 0 100 100"><circle cx="50" cy="50" r="8" fill="currentColor"/><circle cx="50" cy="50" r="34" fill="none" stroke="currentColor" strokeWidth="4" strokeDasharray="10 8"/></svg>;
  if (type === 4) return <svg viewBox="0 0 100 100"><line x1="50" y1="15" x2="50" y2="42" stroke="currentColor" strokeWidth="8"/><line x1="50" y1="58" x2="50" y2="85" stroke="currentColor" strokeWidth="8"/><line x1="15" y1="50" x2="42" y2="50" stroke="currentColor" strokeWidth="8"/><line x1="58" y1="50" x2="85" y2="50" stroke="currentColor" strokeWidth="8"/></svg>;
...
export default function App() {
  ...
  const spawnTarget = (previousId = target.id) => {
    ...
    setTarget({ visible: true, id: spot.id, x: spot.x, y: spot.y });
  ...
  const addSplat = (x, y) => {
    ...
    setTimeout(() => setSplats((old) => old.filter((s) => s.id !== id)), 550);
  };
  ...
  return (
    <main className="game-shell">
      <section className="game-stage" onPointerMove={updatePointer} onPointerDown={(event) => { updatePointer(event); miss(); }}>
        <div className="bg" />
        <div className="dark-vignette" />

        <div className="hud">
          <div className="hud-card"><div className="label">SCORE</div><div className="value">{score}</div></div>
          <div className="hud-card"><div className="label">TIME</div><div className="value">{time}</div></div>
          <div className="hud-card"><div className="label">WAVE</div><div className="value">{wave}</div></div>
        </div>

        <div className="top-right">
          <div className="hud-card"><div className="label">DAMAGE</div><div className="value">{"🍅".repeat(damage)}{"♡".repeat(Math.max(0, MAX_DAMAGE - damage))}</div></div>
          <button className="primary" onPointerDown={(e) => e.stopPropagation()} onClick={start}>{running ? "Restart" : "Start"}</button>
        </div>

        <AnimatePresence>
          {target.visible && (
            <motion.div className="target" style={{ left: `${target.x}%`, top: `${target.y}%` }}
              initial={{ scale: 0.2, y: 35, opacity: 0 }} animate={{ scale: 1, y: 0, opacity: 1 }}
              exit={{ scale: 0.1, y: 25, opacity: 0 }} transition={{ type: "spring", stiffness: 280, damping: 19 }}
        ...
        {tomatoes.map((tomato) => (
          <motion.div key={tomato.id} className="tomato"
            style={{ left: `${tomato.x}%`, top: `${tomato.y}%`, scale: tomato.size }}
            onPointerDown={(event) => hitTomato(event, tomato)} />
        ))}

        {splats.map((splat) => (
          <motion.div key={splat.id} className="splat" style={{ left: `${splat.x}%`, top: `${splat.y}%` }}
            initial={{ scale: 0.15, opacity: 0.95 }} animate={{ scale: 1.45, opacity: 0 }} transition={{ duration: 0.55 }} />
        ))}

        {flash && <div className="flash" />}
        <div className="crosshair" style={{ left: `${pointer.x}%`, top: `${pointer.y}%` }}><CrosshairIcon type={crosshair} /></div>

        <div className="controls" onPointerDown={(e) => e.stopPropagation()}>
          <div className="label">AIM</div>
          {[1, 2, 3, 4, 5].map((id) => (
            <button key={id} className={`crosshair-btn ${crosshair === id ? "active" : ""}`} onClick={() => setCrosshair(id)}>
              <CrosshairIcon type={id} />
            </button>
          ))}
          <button className="crosshair-btn" onClick={() => setSound((s) => !s)}>{sound ? "🔊" : "🔇"}</button>
        </div>
        ...
        {!running && (
          <div className="start-overlay">
            <div className="start-panel">
              ...
              <p>Сбивай летящие помидоры и попадай по всплывающей цели. На мобильном — тапай по целям.</p>
              <button className="primary" onClick={start}>Play</button>
              <br />
              <a className="donate" href="https://www.paypal.com/donate/?hosted_button_id=YOUR_BUTTON_ID" target="_blank" rel="noreferrer">
                💛 Donate via PayPal
              </a>
            </div>
          </div>
